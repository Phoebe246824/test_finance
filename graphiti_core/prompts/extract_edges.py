...
- If both entities are event-like nodes (e.g., NewsEvent) and the text indicates causality
    (e.g., "导致", "引发", "促使", "推高", "压低", "使得", "造成", "因此"), emit a causal edge.
- Prefer creating a directional edge from cause -> effect using the most specific relation type
    available in FACT_TYPES (e.g., Causes). 
- When a multi-step causal chain is present, link adjacent steps as separate edges rather than
    skipping intermediate events.
