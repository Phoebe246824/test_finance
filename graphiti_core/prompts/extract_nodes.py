...
Event Extraction:
- If ENTITY TYPES include event-like types (e.g., NewsEvent), extract event entities that represent
    concrete state changes or causal steps (e.g., supply gaps, price surges, cost increases, demand drops).
- Event entities should capture a subject plus a trigger/action or a clear result/object. Use concise,
    normalized event names when possible (e.g., "Lithium supply gap", "Battery material cost increase").
- When a sentence implies a causal step in a chain, extract the event node even if it is a derived
    business outcome, as long as it is explicitly supported by the text.
...
2. **Entity Identification**:
   ...
   - **Exclude** entities mentioned only in the PREVIOUS MESSAGES (they are for context only).
     - For event entities, prefer names that reflect the causal step (e.g., "price surge", "cost increase",
         "demand decline") over vague nouns.
