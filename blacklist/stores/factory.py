...
from blacklist.stores.events_schema import ANALYZED_EVENTS_COLLECTION, EVENTS_COLLECTION
...
class StoreBundle:
    events: AnalyzedEventsStore
    stashed_events: EventsStore
...
def events_collection_from_config(config: dict[str, Any] | None = None) -> str:
    ...
    return str(
        milvus_config.get("stash_collection")
        or os.getenv("MILVUS_STASH_COLLECTION")
...
def create_store_bundle(config: dict[str, Any]) -> StoreBundle:
    ...
    return StoreBundle(
        events=AnalyzedEventsStore(
            ...
            ttl_days=ttl_days,
            collection_name=ANALYZED_EVENTS_COLLECTION,
