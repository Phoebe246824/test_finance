...
@pytest.mark.asyncio
async def test_event_samples_store_deduplicates_by_content() -> None:
    client = FakeMilvusClient()
    store = EventSamplesStore(client, embedding_fn=fake_embed, embedding_dim=3)

    first = await store.append_event("S001", "AML样本", "疑似分拆交易与洗钱")
    duplicate = await store.append_event("S002", "另一个样本", "疑似分拆交易与洗钱")

    assert first == 1
    assert duplicate == 0
    assert sorted(client.rows["blacklist_event_samples"]) == ["S001"]
