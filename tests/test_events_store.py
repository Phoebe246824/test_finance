...
async def test_upsert_event_creates_events_collection_and_full_row(
    ...
    assert written == 1
    assert client.create_calls[0]["collection_name"] == "stashed_events"
    row = client.rows["stashed_events"]["E001"]
...
async def test_stash_event_deduplicates_by_content_hash(
    ...
    assert list(client.rows["stashed_events"]) == ["E001"]
...
async def test_upsert_event_keeps_pending_duplicate_content_with_new_event_id(
    ...
    assert written == 1
    assert sorted(client.rows["stashed_events"]) == ["PENDING", "STASHED"]
    assert client.rows["stashed_events"]["PENDING"]["status"] == "pending"
