...
def test_build_ppt_metrics_summarizes_cases_and_hardware():
    ...
    assert (
        metrics["p10"]["average_power_watts"]
        is not (metrics["p11"]["optimized"]["average_power_watts"])
    )
...
def test_build_ppt_metrics_uses_planned_sidecar_contract_keys():
            ...
            "p11": {
                "baseline": {
                    ...
                    "risk_level_consistency": {
                        "value": 0.7,
                        "note": "baseline repeated-run consistency",
                    },
                },
                "optimized": {
                    "risk_level_consistency": {
    ...
    assert metrics["p11"]["baseline"]["risk_level_consistency"] == {
        "value": 0.7,
        "unit": "",
        "source": "sidecar",
        "note": "baseline repeated-run consistency",
...
def test_build_ppt_metrics_marks_malformed_sidecar_leaf_not_available():
    metrics = build_ppt_metrics(
        cases=[],
        hardware_samples=[],
        sidecar={
    ...
    assert metrics["p10"]["ttft_ms"] == {
        ...
        "note": "sidecar p10.ttft_ms missing value",
    }
