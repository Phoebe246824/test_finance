...
def test_model_service_test_calls_openai_compatible_models_endpoint(monkeypatch):
    from backend.app.services import model_service_settings
...
def test_model_service_test_rejects_metadata_address_without_request(monkeypatch):
    from backend.app.services import model_service_settings
...
def test_model_service_test_rejects_hostname_that_resolves_to_private_address_without_request(monkeypatch):
    from backend.app.services import model_service_settings
    ...
    monkeypatch.setattr("backend.app.services.url_safety.socket.getaddrinfo", fake_getaddrinfo)
    client = TestClient(create_app(), raise_server_exceptions=False)
...
def test_model_service_test_rejects_malformed_port_without_server_error():
    client = TestClient(create_app(), raise_server_exceptions=False)
...
def test_embedding_model_service_test_posts_to_embeddings_endpoint(monkeypatch):
    from backend.app.services import model_service_settings
...
def test_reranker_model_service_test_posts_to_runtime_rerank_endpoint(monkeypatch):
    from backend.app.services import model_service_settings
