...
const EVENT_DETAIL_RETRY_DELAY_MS = 250
    ...
    http.get(`/api/tasks/${task.task_id}`, { signal }).then(() => {
      ...
      es.addEventListener('update', (e) => {
        ...
        if (current.status === 'success') {
          ...
          const fetchDetail = async (retry = false): Promise<void> => {
            try {
              const { data } = await http.get(`/api/events/${current.event_id}`, { signal })
...
function eventToAnalyzeResult(data: Record<string, unknown>): AnalyzeResult {
  return {
    ...
    graph_result: (data.graph_result || null) as Record<string, unknown> | null,
