...
const serviceFallbacks: ModelService[] = [
  ...
  { name: 'Redis 服务', type: '缓存服务', deployment: 'Docker Compose', endpoint: 'redis://localhost:6379/0', status: '正常', default: false, updatedAt: '' },
  { name: 'Neo4j 图数据库', type: '图数据库', deployment: 'Docker Compose', endpoint: 'bolt://localhost:7687', status: '正常', default: false, updatedAt: '' },
  { name: 'SQLite 数据库', type: '本地数据库', deployment: '本地文件', endpoint: 'data/sentinel_edge.db', status: '正常', default: false, updatedAt: '' },
...
async function load() {
  try {
    ...
    error.value = err?.response?.data?.detail || err?.message || '加载设置失败'
  }
}
